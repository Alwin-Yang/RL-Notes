...
[Policy Optimization](../policy-optimization/index.md) defines what we are
...
which is what [value-based methods](../value-based/index.md) and
[SAC](sac.md) do;
the [Q&A](q-and-a.md) covers that trade-off. But for on-policy advantage
...
[REINFORCE](../policy-optimization/policy-improvement.md) applies to every step.
...
- [A2C / A3C](a2c-a3c.md): the synchronous and asynchronous on-policy versions
  of everything above.
- [Soft Actor-Critic](sac.md): an off-policy actor-critic that learns $Q$ and
  adds an entropy bonus.

## Questions

See the [Actor-Critic Q&A](q-and-a.md) for conceptual questions about what the
critic is for, why it learns $V^\pi$ rather than $Q^\pi$, and how that differs
from Q-learning.
